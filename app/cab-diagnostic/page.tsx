...
import { getCabDiagnostic, submitCabAssessment } from "@/lib/cab.actions";
...
export default function CabPage() {
  ...
  async function handleSubmit() {
    setBusy(true);
    try {
      const r = await submitCabAssessment(Array.from(selected));
  ...
  function reset() { setResult(null); setSelected(new Set()); }
  ...
  return (
    <NudgeShell>
              ...
              >
                <div className="flex items-start gap-3">
                  <div
                    ...
                    style={{ background: isSel ? "#623CEA" : "white", border: isSel ? "none" : "2px solid #C8C5BC" }}
                  >
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t z-40" style={{ borderColor: "#E8E6DC", boxShadow: "0 -2px 12px rgba(34,29,35,0.06)" }}>
        <div className="max-w-3xl mx-auto px-4 py-3 flex items-center justify-between gap-3">
          <span className="text-sm font-semibold" style={{ color: "#221D23" }}>{selected.size} selected</span>
          <button className="n-btn n-btn-primary n-btn-press" disabled={busy} onClick={handleSubmit}>
