...
type Activity = {
  id: string; name: string; weekly_hours: number;
  ai_capable: "yes" | "partly" | "no";
  recommended_tool: string | null; how_to: string | null;
  hours_saved: number; ai_bullets: Bullet[] | null;
  clarity: string | null; commitment: string | null;
};
...
export default function ResultsPage() {
  ...
  async function saveAll() {
    ...
    try {
      await Promise.all(
        activities.map((a) =>
          (supabase.from("activities") as any).update({ commitment: commitments[a.id] ?? "" }).eq("id", a.id)
        )
      );
    ...
    <NudgeShell>
      ...
      <div className="max-w-3xl mx-auto px-4 py-8 md:py-12">
        ...
        <div className="mt-8 flex flex-wrap gap-3 items-center justify-between">
          <Link href="/activities" className="n-btn n-btn-ghost">← Edit activities</Link>
...
function ActivityCard({ a, commitment, onCommitmentChange }: {
  a: Activity; commitment: string; onCommitmentChange: (v: string) => void;
}) {
    ...
    <div className="n-card">
      ...
      <div className="text-sm text-[var(--n-text-muted)] mb-4">
        ...
        {!vague && Number(a.hours_saved) > 0 && (
          <> · <span className="font-semibold text-[var(--n-emerald)]">Estimated time saved: {Number(a.hours_saved).toFixed(1)} hrs/week</span></>
        )}
      </div>
      ...
      {vague ? (
        <div className="rounded-xl p-4 mb-4" style={{ background: "#FFF7DD", border: "1px solid #F5DC7B" }}>
          ...
          <Link href="/activities" className="n-btn n-btn-ghost !py-1.5 text-sm mt-3 inline-block">Edit this activity</Link>
        </div>
      ) : a.ai_capable === "no" ? (
