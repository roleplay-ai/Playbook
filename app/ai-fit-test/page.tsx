...
import { getAiFitTest, submitAiFitTest } from "@/lib/ai-fit.actions";
...
export default function AiFitTestPage() {
  ...
  function start() { setAnswers({}); setIdx(0); setStage("play"); }
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const r = await submitAiFitTest(payload);
  ...
  function retake() { setResult(null); setStage("intro"); }
    ...
    return (
      <NudgeShell>
        <div className="max-w-2xl mx-auto px-4 py-10 md:py-14">
          <Link href="/hub" className="inline-flex items-center gap-1 text-sm text-[var(--n-text-muted)] hover:underline mb-6">
      ...
      <NudgeShell>
        ...
        <div className="max-w-3xl mx-auto px-4 py-10 md:py-14">
          ...
          <div className="flex gap-3 n-reveal n-reveal-delay-3">
            ...
            <Link href="/hub" className="n-btn n-btn-primary n-btn-press">Continue your journey</Link>
          </div>
        </div>
      </NudgeShell>
    );
  ...
  return (
    <NudgeShell>
      <div className="max-w-2xl mx-auto px-4 py-10 md:py-14 pb-32">
        ...
        <h2 className="text-2xl md:text-3xl font-black mb-8 leading-snug" style={{ color: "#221D23" }}>{q.scenario}</h2>
          ...
          {(["use", "partial", "avoid"] as Choice[]).map((c) => {
            ...
            return (
              <button
                ...
                style={{ cursor: "pointer", borderLeft: isSel ? `4px solid ${s.bg}` : "4px solid transparent", background: isSel ? `${s.bg}15` : undefined }}
              >
                <div className="w-10 h-10 rounded-lg flex items-center justify-center font-bold flex-shrink-0" style={{ background: s.bg, color: s.fg }}>
                  {c === "use" ? "✓" : c === "partial" ? "~" : "✗"}
                </div>
        ...
        <div className="flex justify-between mt-8">
          <button className="n-btn n-btn-ghost inline-flex items-center gap-1" disabled={idx === 0} onClick={() => setIdx((i) => Math.max(0, i - 1))}>
            <ArrowLeft size={14} /> Previous
          </button>
          {idx < questions.length - 1 ? (
            <button className="n-btn n-btn-ghost inline-flex items-center gap-1" onClick={() => setIdx((i) => Math.min(questions.length - 1, i + 1))}>
