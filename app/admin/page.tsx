...
import { adminAddCompany, adminDeleteUser, adminGetAll } from "@/lib/admin.actions";
import {
  adminGetCabOverview, adminUpsertCabQuestion, adminDeleteCabQuestion, adminUpsertCabLevelDefinition,
} from "@/lib/cab.actions";
import { adminGetAiFitOverview, adminUpsertAiFitQuestion, adminDeleteAiFitQuestion } from "@/lib/ai-fit.actions";
...
function AdminPageInner() {
  ...
  const initialTab: AdminTab = rawTab === "cab" || rawTab === "fit" || rawTab === "audit" ? rawTab : "audit";
  ...
  useEffect(() => {
    ...
    if (!profile?.is_admin) { router.push("/activities"); return; }
    refresh();
  ...
  async function handleAddCompany() {
    ...
    const r = await adminAddCompany({ name: newCompany.trim() });
  ...
  async function handleDeleteUser(uid: string, username: string) {
    ...
    const r = await adminDeleteUser({ user_id: uid });
    if (!r.ok) return toast.error(r.error || "Failed");
          ...
          <div>
            ...
            <div className="n-card mb-8 overflow-x-auto">
              <table className="w-full text-sm">
                ...
                <tbody>
                  {(() => {
                    const tableRows = filteredProfiles.map(p => {
                    ...
                    return tableRows.map(({ p, acts, spent, saved, compName }) => (
...
export default function AdminPage() {
  return (
    <Suspense fallback={<div className="p-12 text-center text-[var(--n-text-muted)]">Loading…</div>}>
      <AdminPageInner />
    </Suspense>
  );
}
...
function CabAdmin({ companies, profiles }: { companies: Company[]; profiles: Profile[] }) {
  ...
  async function refresh() {
    setBusy(true);
    try {
      const r = await adminGetCabOverview();
...
function QuestionsAdmin({ questions, onChange }: { questions: CabQuestion[]; onChange: () => void }) {
  ...
  async function remove(id: string) {
    ...
    try { await adminDeleteCabQuestion({ id }); toast.success("Deleted"); onChange(); }
    catch (e) { toast.error((e as Error).message); }
  }
...
function LevelsAdmin({ levels, onChange }: { levels: CabLevelDef[]; onChange: () => void }) {
  ...
  async function save() {
    if (!editing) return;
    try {
      await adminUpsertCabLevelDefinition({
...
function FitAdmin({ profiles, companies }: { profiles: Profile[]; companies: Company[] }) {
  ...
  async function save() {
    if (!editing) return;
    try {
      await adminUpsertAiFitQuestion({ ...editing, id: editing.id || undefined });
      toast.success("Saved");
      setEditing(null);
      load();
    } catch (e) { toast.error((e as Error).message); }
  ...
  async function remove(id: string) {
    ...
    try { await adminDeleteAiFitQuestion({ id }); load(); } catch (e) { toast.error((e as Error).message); }
  }
    ...
    <div>
      ...
      {subtab === "overview" ? (
        <div>
          ...
          {attempts.length === 0 && <p className="text-sm text-[var(--n-text-muted)]">No attempts yet.</p>}
        </div>
      ) : subtab === "users" ? (
        <div>
          <div className="flex justify-between items-end mb-4 gap-3 flex-wrap">
