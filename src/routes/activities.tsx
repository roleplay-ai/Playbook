...
import { recommendTools } from "@/lib/anthropic.functions";
...
function ActivitiesPage() {
  ...
  const recommendFn = useServerFn(recommendTools);
    ...
    supabase.from("activities").select("*").eq("user_id", user.id).order("created_at").then(({ data }) => {
      if (data && data.length > 0) {
        setRows(data.map(a => ({
          ...
          classification: (a.ai_capable === "yes" || a.ai_capable === "partly" || a.ai_capable === "no") ? a.ai_capable : null,
        })));
      }
    });
  }, [loading, user, profile, navigate]);

  // Debounced autosave of complete rows
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      if (payload.length > 0) await supabase.from("activities").upsert(payload);
  ...
  useEffect(() => {
    ...
    const t = setTimeout(() => {
      supabase.from("profiles").update({ role: value }).eq("id", user.id);
    }, 400);
    return () => clearTimeout(t);
  ...
  async function handleGenerate() {
    ...
    try {
      ...
      if (reqs.length > 0) {
        const { results } = await recommendFn({ data: { activities: reqs } });
        await Promise.all(results.map(r => {
          ...
          return supabase.from("activities").update(update).eq("id", r.id);
        }));
      }
      navigate({ to: "/results" });
    } catch (e) {
