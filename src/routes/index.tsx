...
import { ensureAdmin, enterAudit, listCompaniesPublic, usernameToEmail } from "@/lib/auth.functions";
...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Learn where you are, where AI fits, and where AI can create real value in your work." },
      { property: "og:title", content: "AI for Work Playbook" },
      { property: "og:description", content: "A practical journey from random AI use to confident AI application at work." },
    ],
  }),
});
...
function IndexPage() {
  ...
  const ensureAdminFn = useServerFn(ensureAdmin);
  const listCompaniesFn = useServerFn(listCompaniesPublic);
  const enterFn = useServerFn(enterAudit);
  ...
  useEffect(() => { ensureAdminFn().catch(() => {}); }, [ensureAdminFn]);
  ...
  async function handleEnter(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await enterFn({ data: { email, company_id: companyId } });
        ...
        <div className="grid lg:grid-cols-[1.3fr_1fr] gap-10 items-start mb-16">
          <section>
            ...
            <p className="text-[var(--n-text-muted)] text-lg mb-8 max-w-xl">
              A practical journey to help professionals move from random AI use to confident
              AI application at work.
            </p>
          </section>
