...
type Reco = {
  id: string; tool: string; how_to: string;
  bullets: AiBullet[]; total_saved: number; clarity: "clear" | "vague";
};
...
async function recommendOne(act: z.infer<typeof Input>["activities"][number]): Promise<Reco> {
  ...
  if (vague) return { id: act.id, tool: "", how_to: "", bullets: [], total_saved: 0, clarity: "vague" };
...
{"clarity":"clear|vague","tool":"...","bullets":[{"text":"...","hours_saved":0.5}]}`;
  ...
  try {
    ...
    const total_saved = Math.min(bullets.reduce((s, b) => s + b.hours_saved, 0), act.weekly_hours);
    const how_to = bullets.map((b) => `• ${b.text} (~${b.hours_saved.toFixed(1)} hrs/wk)`).join("\n");
    return { id: act.id, tool: parsed.tool ?? "", how_to, bullets, total_saved, clarity };
  } catch (e) {
...
export async function recommendTools(activities: z.infer<typeof Input>["activities"]) {
  await requireAuth();
  const data = Input.parse({ activities });
