...
export async function getPlaybookSummary() {
  ...
  return { cab: cab ?? null, fit: fit ?? null, opportunityCount: oppCount ?? 0 };
}

export async function getAdminPlaybookOverview() {
  const { user } = await requireAuth();
  ...
  const [{ count: totalUsers }, { data: cabAssessments }, { data: fitAttempts }, { data: activities }] =
    await Promise.all([
  ...
  const avgFit = fitScores.length ? fitScores.reduce((s, f) => s + f.score, 0) / fitScores.length : 0;
  const fitTotal = fitScores[0]?.total ?? 15;

  const missCount = new Map<string, { scenario: string; misses: number }>();
  (fitAttempts ?? []).forEach((att) => {
    const arr = (Array.isArray(att.answers) ? att.answers : []) as Array<{ scenario?: string; ok?: boolean }>;
    arr.forEach((b) => {
  ...
  return { totalUsers: totalUsers ?? 0, cabDistribution, avgFit, fitTotal, topMissed, highPriorityOpps };
}
